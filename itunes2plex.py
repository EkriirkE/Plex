...
def low(s): return s.lower() if s else None
	...
	for t in tracks.findall("key"):
		...
		if f:
			r=db.execute("""SELECT md.guid
					...
					WHERE LOWER(file)=?""",(low(f),)).fetchall()
			if r:
				q+=[x["guid"] for x in r]
		...
		if not q or AllMatches:
			r=db.execute("""SELECT ttl.guid,art.guid art_guid,alb.guid alb_guid
					...
					WHERE LOWER(ttl.title)=? AND LOWER(art.title)=? AND LOWER(alb.title)=?""",(low(d.get("Name")),low(d.get("Artist") or d.get("Genre")),low(d.get("Album")))).fetchall()
			if r:
		...
		if not q or AllMatches:
			r=db.execute("""SELECT ttl.guid,art.guid art_guid
					FROM metadata_items ttl
					INNER JOIN metadata_items art ON art.id=ttl.parent_id
					WHERE LOWER(ttl.title)=? AND LOWER(art.title)=?""",(low(d.get("Name")),low(d.get("Artist") or d.get("Genre")))).fetchall()
			if r:
				q+=[x["guid"] for x in r if x["guid"] not in q]
		...
		for guid in q or []:
			for a in profiles:
				...
				if e:=db.execute("SELECT * FROM metadata_item_settings WHERE account_id=? AND guid=?",(a["id"],guid)).fetchone():
					#Highest viewcount
					if not pc or (e["view_count"] and pc and e["view_count"]>pc): pc=e["view_count"]
					#Most recent play date
					if not pd or (e["last_viewed_at"] and pd and e["last_viewed_at"]>pd): pd=e["last_viewed_at"]
					#Earliest creation date
					if not da or (e["created_at"] and da and e["created_at"]<da): da=e["created_at"]
					#Most recent modified date
					if not dm or (e["updated_at"] and dm and e["updated_at"]>dm): dm=e["updated_at"]
					#Most recent rated date
					if not rd or (e["last_rated_at"] and rd and e["last_rated_at"]>rd): rd=e["last_rated_at"]
					#Average rating.  elif?
					if not rt: rt=e["rating"]
					elif e["rating"] and rt: rt=(rt+e["rating"])/2
					q="UPDATE metadata_item_settings SET rating=?,last_viewed_at=?,view_count=?,created_at=?,updated_at=?,last_rated_at=? WHERE account_id=? AND guid=?",(rt,pd,pc,da,dm,rd,a["id"],guid)
				else:
					q="INSERT INTO metadata_item_settings (account_id,guid,rating,view_count,last_viewed_at,created_at,updated_at,last_rated_at) VALUES (?,?,?,?,?,?,?,?)",(a["id"],guid,rt,pc,pd,da,dm,rd)
					eprint("+",end="",flush=True)
